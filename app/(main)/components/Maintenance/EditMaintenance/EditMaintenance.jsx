...
export default function EditMaintenances({ maintenancesId }) {
    ...
    function editMaintenances(event) {
        ...
        // SEND THE REQUEST
        axios.put(`${process.env.API_URL}/edit/maintenance`, formData, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
            .then(response => {
                toast.success(response.data?.message || 'تم تعديل الزيارة بنجاح.');
                setLoading(false);
            })
            .catch(error => {
                toast.error(error?.response?.data?.message || 'حدث خطأ أثناء تعديل الزيارة.');
                setLoading(false);
            });
    }


    // GET THE MAINTENANCES FROM THE DATABASE
    function getMaintenance(id) {
        // GET THE TOKEN FROM THE COOKIES
        const token = localStorage.getItem('token');

        axios.get(`${process.env.API_URL}/admin/maintenance?maintenanceId=${id}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
            ...
            .catch(err => {
                toast.error(err?.response?.data?.message || 'حدث خطأ أثناء جلب بيانات الزيارة.');
            });
    }
    ...
    return (
        <div className={'card mb-0'} dir={'rtl'}>
            <h1 className={'text-2xl font-bold mb-4 uppercase'}>تعديل الزيارة </h1>
